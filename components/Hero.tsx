export default function Hero() {
  return (
    <section id="about" className="grid grid-cols-1 lg:grid-cols-12 gap-4">
      ...
      {/* Main headline card — neutral dark */}
      <div className="lg:col-span-8 bg-[#262626] rounded-[32px] p-8 lg:p-10 relative overflow-hidden min-h-[380px] flex flex-col justify-between">
        ...
        <div className="flex items-center gap-3 mt-8 relative">
          <a
            href="#work"
            className="inline-flex items-center gap-2 bg-[#f0f0f0] text-[#111111] font-sans font-semibold text-sm px-6 py-3 rounded-full hover:bg-white transition-colors"
          >
            View Work
          </a>
          <a
            href="#contact"
            className="inline-flex items-center gap-2 bg-white/8 text-white/70 font-sans text-sm px-6 py-3 rounded-full hover:bg-white/12 transition-colors border border-white/10"
          >
            Get in Touch
          </a>
        </div>
      ...
      {/* Role card — dark navy */}
      <div className="lg:col-span-4 bg-[#1e3050] rounded-[32px] p-8 flex flex-col justify-between relative overflow-hidden min-h-[280px]">
      ...
      {/* Stack card — dark grey */}
      <div className="lg:col-span-4 bg-[#1a1a1a] rounded-[32px] p-8">
        ...
        <div className="flex flex-wrap gap-2">
          {['C++', 'Python', 'React', 'Next.js', 'TypeScript', 'Node.js', 'Figma'].map((skill) => (
            <span
              key={skill}
              className="font-sans text-xs text-white/60 bg-white/8 border border-white/10 px-3 py-1.5 rounded-full"
            >
              {skill}
            </span>
          ))}
      ...
      {/* Bio card — dark grey */}
      <div className="lg:col-span-5 bg-[#222222] rounded-[32px] p-8 flex flex-col justify-between">
      ...
      {/* Experience card — dark muted purple */}
      <div className="lg:col-span-3 bg-[#2a1f42] rounded-[32px] p-8 flex flex-col justify-between">
      ...
      {/* UI/UX Portfolio bento — dark muted rose */}
      <a
        href="https://reymartlouie.framer.website"
        target="_blank"
        rel="noopener noreferrer"
        className="lg:col-span-12 bg-[#3a1f2c] rounded-[32px] p-8 flex flex-col md:flex-row items-start md:items-center justify-between gap-6 relative overflow-hidden group"
      >
        <div className="absolute -right-20 -top-20 w-72 h-72 bg-rose-400/10 rounded-full blur-3xl pointer-events-none" />
        <div className="absolute left-1/3 bottom-0 w-48 h-48 bg-pink-400/8 rounded-full blur-2xl pointer-events-none" />
        <div className="relative">
        ...
        <span className="relative inline-flex items-center gap-2 bg-[#fbcfe8]/12 text-[#fbcfe8] border border-[#fbcfe8]/20 font-sans font-semibold text-sm px-6 py-3 rounded-full group-hover:bg-[#fbcfe8]/20 transition-colors whitespace-nowrap">
          View Portfolio ↗
        </span>
      </a>
