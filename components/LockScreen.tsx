...
const SPRING   = 'transform 500ms cubic-bezier(0.34,1.1,0.64,1)'
...
export default function LockScreen({ onUnlock }: { onUnlock: () => void }) {
  ...
  // Any keydown while idle → activate; Escape → deactivate
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      if (exiting) return
      if (e.key === 'Escape') { deactivate(); return }
      if (!inputOpen) activate()
    }
  ...
  return (
    <div
      ...
      style={{
        ...
        transition: exiting ? 'opacity 600ms cubic-bezier(0.4,0,0.2,1)' : 'opacity 400ms ease',
    ...
    >
      ...
      <div
        ...
        style={{
          ...
          transition: 'top 500ms cubic-bezier(0.34,1.1,0.64,1)',
        }}
      >
        <p
      ...
      <div
        className="fixed left-1/2 bottom-7 w-full px-4"
        style={{
          maxWidth:  '300px',
          transform: centered
            ? 'translateX(-50%) translateY(calc(-50vh + 1.75rem + 50%))'
            : 'translateX(-50%)',
          transition: SPRING,
        }}
      ...
      >
        <div
          className="flex flex-col items-center gap-5 px-8 py-8 w-full rounded-[32px]"
          style={{
        ...
        >
          {/* Avatar */}
          <button
            onClick={!inputOpen ? activate : undefined}
